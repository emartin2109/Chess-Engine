...
void generate_bishop_lookup_table(void)
{
    ...
    lookup_table_bishop = malloc(64 * sizeof(long long unsigned int));
    int indexes[64];
    for (int i = 0; i < 64; i++) {
        unsigned long long int max_index_in_lookup = 0;
        recursive_params.enemies = 0;
        recursive_params.allies = 0;
        lookup_table_bishop[i] = NULL;
        int number_of_ones = 0;
    
        moves_t moves = generate_sliding_moves(precomputed_values.power[i], true, false, &recursive_params);
        ...
        for (int pattern_index = 0; pattern_index < nbr_of_patterns; pattern_index++) {
            ...
            // printf("\n");
            unsigned long long int index_in_lookup = (move * BishopMagics[i]) >> BishopShifts[i];
            if (index_in_lookup >= max_index_in_lookup) {
                max_index_in_lookup = index_in_lookup;
                lookup_table_bishop[i] = realloc(lookup_table_bishop[i], (max_index_in_lookup + 1) * sizeof(long long unsigned int));
            }
            recursive_params.enemies = move;
            moves = generate_sliding_moves(precomputed_values.power[i], true, false, &recursive_params);
            lookup_table_bishop[i][index_in_lookup] = moves.moves | moves.captures;
        }
    }
}
